...
public final class OpenWeatherMapClient {
	...
	private static String WEATHER_URL = "http://api.openweathermap.org/data/2.5/weather?appid=%s&q=%s&units=metric";

	private static String WEATHER_URL_HTTPS = "https://api.openweathermap.org/data/2.5/weather?appid=%s&q=%s&units=metric";

	private static String UVI_URL = "http://api.openweathermap.org/data/2.5/uvi?appid=%s&lat=%s&lon=%s";
	...
	public static CurrentWeatherSummary showCurrentWeather(String city) {
		...
		try {
			WeatherData weatherData = RestTemplateProxy.INSTANCE.getRestTemplate()
					.getForObject(String.format(WEATHER_URL, APP_KEY, city), WeatherData.class);
	...
	private static void testURL() {
		try {
			RestTemplateProxy.INSTANCE.getRestTemplate().getForObject(String.format(WEATHER_URL, APP_KEY, DEFAULT),
					WeatherData.class);
		} catch (Exception ex) {
			WEATHER_URL = WEATHER_URL_HTTPS;
			UVI_URL = UVI_URL_HTTPS;
		}
		LOGGER.info("URL = " + WEATHER_URL);
	}
}
