...
public final class OpenWeatherMapClient
{
    ...
    public static CurrentWeatherSummary showCurrentWeather(String c)
    {
        ...
        try
        {
            getCurrentWeatherSummary(API_URL_HTTP, city, summary);
        }
        catch (Exception e)
        {
            LOGGER.error("Failed to get the current weather data form OpenWeatherMap.", e);
            getCurrentWeatherSummary(API_URL_HTTPS, city, summary);
        }

        return summary;
    }

	private static void getCurrentWeatherSummary(String url, String city, CurrentWeatherSummary summary) {
